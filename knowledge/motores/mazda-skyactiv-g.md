# Skyactiv-G — 2.0 y 2.5 atmosféricos (Mazda)

| | |
|---|---|
| **Montado en** | Mazda 3 (2014-2018), Mazda 6, CX-5 |
| **Veredicto** | 🟢 [V8] prefiere el **2.5** 🟡 |

## Por qué

Fiables y eficientes a la vez. [V8] llama a Skyactiv "una obra maestra de
ingeniería" 🟡. La automática de seis marchas que los acompaña la considera de
**las mejores del mercado** en esos años: rápida, suave y duradera 🟡 — dato
poco común, porque la caja suele ser el punto débil en este segmento.

## El mantenimiento que no se puede saltar

Es de **inyección directa**, y eso trae una consecuencia concreta:

> **Carbonilla en las válvulas de admisión.** Limpieza profunda periódica.

⚠️ **Las dos fuentes del mismo canal no coinciden en el intervalo:**

| Fuente | Intervalo |
|---|---|
| [V8] (Mazda 3) | cada **100.000 km como mínimo** 🟡 |
| [V10] (CX-5) | revisar la admisión hacia los **200.000 km** 🟡 |

[V10] aporta además el encuadre correcto: **no es una avería, es un
mantenimiento caro a largo plazo** que hay que presupuestar 🟡.

Al responder, dar el rango y decir que las fuentes discrepan — no inventar un
número intermedio.

En inyección directa el combustible no pasa por las válvulas de admisión, así
que no las lava como hacía la inyección indirecta. Los depósitos se acumulan y
no se van solos.

**Conecta con [V7]**, que recomienda montar un **catch can** en todos los
motores de inyección directa 🟡: la porquería que ese vaso retiene es
exactamente la que acaba pegada en esas válvulas. Ver
[aceite y lubricación](../../references/aceite-y-lubricacion.md).

> ⚠️ **Corregido el 2026-09-09.** Esto se marcó ✅ dando por hecho que [V7] y
> [V8] eran canales distintos. **No lo son.** Es una sola fuente hablando del
> mismo problema dos veces, así que vuelve a 🟡. Sigue siendo un consejo
> coherente; simplemente no está confirmado.

## Fuentes
- `[V8]`, `[V10]`, `[V7]`
